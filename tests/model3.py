...
class CNN_Net2(nn.Module):
	def __init__(self):
		...
		self.conv_block_cap = Conv_block()
		self.conv_block_ncap = Conv_block()
		self.residual_blocks_cap = self.__make_residual_blocks()
		self.residual_blocks_ncap = self.__make_residual_blocks()
	...
	def forward(self, x, is_capture):
		...
		if is_capture:
			out = self.conv_block_cap(x)
			out = self.residual_blocks_cap(out)
			value = self.value_capture_head(out)
			return value
		else:
			out = self.conv_block_ncap(x)
			out = self.residual_blocks_ncap(out)
